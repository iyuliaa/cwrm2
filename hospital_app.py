...
#
# Research Question:
#   What is the relative burden of hospital admission categories across ICD chapters and diagnoses, and how did the most important diagnoses change relative to their pre-COVID baseline during the 2020–21 lockdown period? ?
#
# Visual Design:
...
.chapter-legend-subtitle {
    font-size: 0.82rem;
    color: #64748b;
    margin-bottom: 0.75rem;
    line-height: 1.45;
}
/* Fixed 5-column grid — always neat regardless of wrapping */
.chapter-legend-grid {
    display: grid;
...
.chapter-legend-item span.label {
    overflow: hidden;
    text-overflow: ellipsis;
}
/* Colour swatch — larger and more visible */
...
SHORT_LABELS = {
    ...
    'Health services in circumstances related to reproduction':      'Repro Health',
    ...
    'Complications of surgical & medical care nec.':                 'Care Compli',
    ...
    'Other diseases of the urinary system': 'Disease Urinary',
    'Soft tissue disorders': 'Soft Dis.',
    'Examination and investigation': 'Exam Inv',
    'Noninflammatory disorders of female genital tract': 'Noninflam Dis',
    'Malignant neoplasm of breast': 'Breast cancer',
    'Malignant neoplasm of digestive organs': 'GI Cancer'
}
...
def chapter_legend_html(color_map: dict, chapter_totals: pd.Series) -> str:
    """Build a tidy grid-based HTML legend mapping each ICD chapter to its colour.

    Chapters are ordered from highest to lowest total admissions.
    Five items per row via CSS grid — always aligned regardless of label length.
    """
    items = []
    for chapter in chapter_totals.sort_values(ascending=False).index:
        ...
        # Format total: show as e.g. "12.3M" or "987K"
        if total >= 1_000_000:
            total_str = f"{total/1_000_000:.1f}M"
        else:
            total_str = f"{total/1_000:.0f}K"
        items.append(
            f'<div class="chapter-legend-item">'
            f'<span class="chapter-legend-swatch" style="background:{color};"></span>'
            f'<span class="label"><b style="color:#0f172a">{chapter}</b>'
            f'<span style="color:#94a3b8; font-size:0.78rem; margin-left:4px;">({total_str})</span></span>'
            f'</div>'
        )

    return f"""
    <div class="chapter-legend">
      <div class="chapter-legend-title">Treemap Colour Legend — ICD Chapters</div>
      <div class="chapter-legend-subtitle">
        Each colour represents one ICD disease chapter. Darker shades within a chapter indicate
        higher admission volumes; lighter shades indicate lower volumes.
        Chapters are ordered by total burden (highest → lowest) and only larger diagnoses are labelled for readability (>2M). Admission totals shown in brackets.
      </div>
...
#   • Treemap  — ★ rank labels and black cell borders
...
st.markdown("""
<div class="rq-box">
<b>Research Question:</b> What is the relative burden of hospital admission categories across ICD chapters and diagnoses, and how did the most important diagnoses change relative to their pre-COVID baseline during the 2020–21 lockdown period? <br>
...
st.markdown(
    ...
    'Main Figure: Treemap - Admission Burden Is Concentrated Differently Across ICD Chapters'
    '</div>',
    unsafe_allow_html=True
)
...
MIN_DIAG_FRACTION = 0.0115   # ~1.15% of all admissions (~2.0M over 11 years)
...
def build_display_label(row) -> str:
    count_fmt = f"{row['total']:,.0f}"

    if row['show_label']:
        if row['is_top10']:
            rank = top10_rank[row['label_full']]
            return f"★#{rank} {row['label_short']}<br>{count_fmt}"
        return f"{row['label_short']}<br>{count_fmt}"
...
for _, row in tree_df.iterrows():
    ...
    # Top-10 cells use black label text to stand out against dark borders
    if is_top10:
        txt_col = '#fef08a'
...
(highlighted in treemap with black labels and dark borders · these exact diagnoses form the
...
st.markdown(
    '<div class="section-label">'
    'Supporting Figure: Heatmap - High-Burden Diagnoses Responded Unevenly During COVID Lockdown'
    '</div>',
    unsafe_allow_html=True
)
...
fig_hm.add_trace(go.Heatmap(
    ...
    colorbar=dict(
        title=dict(text='% vs<br>2019–20<br>baseline', font=dict(size=18)),
...
st.markdown("""
<p style="font-size:1.82rem; color:#475569; text-align:center; margin-top:-0.5rem">
...
displayed with black label text prefixed by ★#1 through ★#10 and outlined with dark
