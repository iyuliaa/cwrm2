...
# ═══════════════════════════════════════════════════════════════════════════════
# MAIN FIGURE — TREEMAP (DENGAN LEGENDA SKALA WARNA VIRIDIS)
# ═══════════════════════════════════════════════════════════════════════════════
...
**Colour Scale** = Intensity of admissions (Viridis). Hover over any block to see exact totals.
""")
...
# Modifikasi: Menggunakan color='total' dan color_continuous_scale untuk memunculkan Legenda (Scale)
fig_tree = px.treemap(
    tree_df,
...
and diagnosis (inner rectangles). Rectangle area and color intensity are proportional to number of admissions. 
</p>
""", unsafe_allow_html=True)
...
fig_hm.add_trace(go.Heatmap(
    ...
    text=annot.values,
    texttemplate="%{text}",
    # Ukuran besar sesuai permintaan sebelumnya
...
fig_hm.update_layout(
    height=850,  # Tinggi ditingkatkan untuk akomodasi font besar
...
• Colour (Intensity) → total admissions (Viridis scale: purple = low, yellow = high)<br>
...
6. Computed % change = (year_value − 2019-20_value) / 2019-20_value × 100 per diagnosis<br>
7. Applied ±30/−60% clip on colour scale to prevent outliers washing out the palette<br>
8. Created short-label dictionary to shorten y-axis diagnosis names in the heatmap
