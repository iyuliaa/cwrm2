...
def load_data():
    ...
    # Canonical diagnosis labels: fixes inconsistent labels across years
    ...
    def get_chapter(code):
        icd_letter = extract_icd_letter(code)

        if icd_letter is None:
            return 'Other'

        return chapter_map.get(icd_letter, 'Other')
...
def build_chapter_admission_ranges(tree_df_input):
    ...
    for ch, grp in tree_df_input.groupby('chapter'):
        ch_min = grp['total'].min()
        ch_max = grp['total'].max()
        ranges[ch] = (ch_min, ch_max)

    return ranges


# ─── Helpers ──────────────────────────────────────────────────────────────────
def make_short_label(text: str):
    if pd.isna(text):
        return ""
...
def chapter_legend_html(color_map: dict, chapter_totals: pd.Series) -> str:
    items = []
    ordered_chapters = chapter_totals.sort_values(
    ...
    Hue denotes ICD chapter membership. Within each chapter, colour intensity represents relative admission magnitude:
    darker, more saturated tiles indicate higher admission volumes, whereas lighter tiles indicate lower admission volumes.
    Legend entries are ordered by total chapter burden, from highest to lowest.
    </span>
...
top10_burden_list = diag_burden.nlargest(10).index.tolist()
...
diagnosis that suffered the <b>largest lockdown drop</b> (left) to the most resilient</b> (right).
...
fig_hm.add_trace(go.Heatmap(
    ...
    hovertemplate='<b>%{x}</b><br>Year: %{y}<br>Change vs baseline: %{customdata:+.1f}%<extra></extra>',
    xgap=2,
    ygap=2,
))
...
The treemap shows all ICD disease chapters and diagnosis groups nested within each chapter.
The ten highest-burden diagnoses are explicitly marked with ★ rank badges in the treemap.
These exact ten diagnoses form the columns of the supporting heatmap.

<br><br>
<b>Variables:</b><br>
• <b>Total admissions</b> — mapped to rectangle area in the treemap.<br>
• <b>ICD chapter</b> — mapped to categorical colour in the treemap.<br>
• <b>Diagnosis description</b> — identifies specific admission categories.<br>
• <b>Financial year</b> — forms the row axis of the heatmap.<br>
• <b>Percentage change relative to the 2019–20 baseline</b> — mapped to heatmap colour.

<br><br>
<b>Visual Mappings:</b><br>
<u>Treemap:</u><br>
• Hierarchy → outer rectangles = ICD chapters; inner rectangles = diagnosis groups.<br>
• Area → total admissions; larger = greater burden.<br>
• Colour hue → ICD chapter.<br>
• Colour intensity → relative admission magnitude within each chapter.<br>
• ★ rank label → top-10 highest-burden diagnoses.<br>

<u>Heatmap:</u><br>
• X-axis → top-10 burden diagnoses from the treemap.<br>
• Y-axis → financial year.<br>
• Cell colour → % change vs 2019–20 baseline.<br>
• Orange dashed border → 2020–21 COVID lockdown year.<br>
• Cell annotations → exact % values.

<br><br>
<b>Unique Observation:</b><br>
The treemap identifies the ten highest-burden diagnoses across ICD chapters; the heatmap tests whether high cumulative burden
corresponded to resilience during the COVID lockdown. The answer is no — burden and resilience are not aligned.
Labour and Delivery fell only modestly, while elective or deferrable categories such as Arthropathies fell much more sharply.

<br><br>
<b>Data Preparation:</b><br>
The dataset was cleaned by standardising column names, converting numeric fields, removing the partial 2023–24 year,
and dropping rows with missing diagnosis or year values. Diagnosis labels were canonicalised to prevent the same diagnosis
from being split across years due to punctuation or spacing inconsistencies. ICD chapter was derived from the first letter
of the diagnosis code. Admissions were aggregated by chapter and diagnosis for the treemap. Annual % change was computed
relative to the 2019–20 pre-COVID baseline.

<br><br>
<b>Why this design is effective:</b><br>
The treemap compactly shows hierarchical burden across ICD chapters and diagnoses, while the heatmap adds the temporal
dimension needed to assess lockdown disruption and recovery. Together, the figures answer complementary questions:
Figure 1 asks "how much?" and Figure 2 asks "how resilient?"
