...
SHORT_LABELS = {
    ...
    'Health services in circumstances related to reproduction': 'Reproduction care',
    'Diseases of veins & lymphatic system nec.': 'Veins & lymphatics',
    'Other infections and disorders of the skin and subcutaneous tissue': 'Other skin infections',
    'Other conditions originating in the perinatal period': 'Other perinatal cond.'
}
...
# tampilkan label hanya untuk kotak yang cukup besar
threshold = tree_df['total'].quantile(0.55)
tree_df['label_box'] = np.where(
    tree_df['total'] >= threshold, tree_df['display_short'], '')

fig_tree = px.treemap(
    tree_df,
    path=['chapter', 'label_box'],
    values='total',
    color='chapter',
...
and diagnosis (inner rectangles). Rectangle area is proportional to number of admissions, while colour distinguishes disease chapter.
</p>
""", unsafe_allow_html=True)
...
The <b>Digestive</b> chapter occupies the largest overall area, indicating the greatest cumulative burden. However, the largest
single diagnosis block is <b>Labour & Delivery</b>, which sits inside the much smaller <b>Pregnancy & Childbirth</b> chapter.
This contrast shows that some chapters are large because they contain many substantial diagnoses, while others are dominated
by one especially large diagnosis. <b>Eye & Ear</b>, for example, is heavily dominated by <b>Cataracts</b>.
</div>
""", unsafe_allow_html=True)
...
st.markdown("""
Rows = financial year. Columns = the same top-10 diagnoses highlighted by burden in the treemap.  
...
fig_hm.update_layout(
    height=420,
    margin=dict(l=10, r=150, t=20, b=60),
    paper_bgcolor='white',
    plot_bgcolor='white',
    font=dict(family='IBM Plex Sans'),
...
The orange-bordered <b>2020–21</b> row is dominated by deep red cells, showing a broad lockdown disruption. The darkest red
cells correspond to <b>Arthropathies</b> and <b>Cataracts</b>, indicating some of the strongest declines, while <b>Labour & delivery</b>
shows only a modest fall. This suggests that high-burden diagnoses linked to more elective procedures were disrupted more
strongly than less deferrable care.
</div>
""", unsafe_allow_html=True)
...
Treemap: All ICD-10 chapters with individual diagnoses nested inside.<br>
Heatmap: Top 10 highest-burden diagnoses selected from the treemap.

<br><b>Variables & Why Chosen:</b><br>
• Total Admissions (2012–2023) — primary measure of clinical burden, mapped to area in treemap<br>
• ICD Chapter — provides the hierarchy for the outer treemap level<br>
• Diagnosis description — specific condition detail inside each chapter<br>
• % Change vs 2019–20 baseline — isolates the lockdown shock<br>
• Financial Year — time axis in heatmap to show temporal change
...
• Labels → short labels shown only for larger diagnosis blocks; full names appear on hover<br>
...
The treemap shows that although Digestive is the largest overall chapter, the largest single diagnosis block is Labour & delivery
inside the smaller Pregnancy & Childbirth chapter. The heatmap then shows that disruption during the 2020–21 lockdown was
not uniform: Arthropathies and Cataracts show some of the darkest red cells, while Labour & delivery declines much less.
Together, the two visuals indicate that high-burden categories linked to more elective procedures were disrupted more strongly
than less deferrable care.

<br><b>Data Preparation:</b><br>
1. Renamed raw column headers for easier processing<br>
2. Converted admissions-related fields to numeric format<br>
3. Removed incomplete year 2023–24<br>
4. Removed rows with missing year or diagnosis description<br>
5. Derived ICD chapter from the first letter of diagnosis code<br>
6. Aggregated admissions by chapter and diagnosis for the treemap<br>
7. Selected the ten highest-burden diagnoses for the heatmap<br>
8. Calculated % change relative to the 2019–20 baseline<br>
9. Shortened long diagnosis labels for readability
