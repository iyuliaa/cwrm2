...
def load_data():
    ...
    chapter_map = {
        ...
        'E': 'Endo Metabolic', 'F': 'Mental Health',
...
SHORT_LABELS = {
    ...
    'Endocrine nutritional and metabolic diseases': 'Metabolic diseases',
    ...
    'Other Diseases & disorders of the nervous syst.': 'Nervous Dis',
...
def chapter_tinted(hex_color: str, t: float) -> str:
    ...
    t = 0.0 → very pale tint
    t = 1.0 → full saturated hue
    """
    MIN_MIX = 0.20
    MAX_MIX = 1.00
...
def chapter_legend_html(color_map: dict, chapter_totals: pd.Series) -> str:
    ...
    <span style="font-size:0.79rem; color:#64748b;">
     Color indicates the ICD chapter group. Within each chapter, darker and more intense colors represent a higher number of hospital admissions, while lighter colors represent fewer admissions.
...
<span style="font-size:0.82rem; color:#94a3b8">ONS Hospital Admissions Dataset · 2012–2023 ·</span>
</div>
""", unsafe_allow_html=True)
...
hm_rows = {}
yr_totals_all = {
    yr: df[df['year'] == yr].groupby('description')['Admissions'].sum()
    for yr in years
}
