...
/* ─── ADDED: chapter color legend for treemap ───────────────────────────── */
.chapter-legend {
    background: #ffffff;
    border: 1px solid #e2e8f0;
    border-radius: 8px;
...
def load_data():
    ...
    # ─── FIX START: robust ICD parsing ───────────────────────
    def extract_icd_letter(code):
        """
        Extract the first valid ICD chapter letter from messy strings such as:
        '‡ O10-O75, O85-O92, O94-O99' -> 'O'
        ' O80-O84' -> 'O'
        'P00-P04 Fetus and newborn ...' -> 'P'
        """
        ...
        # Best case: letter followed by a digit, e.g. O10, K25, M16
        match = re.search(r'\b([A-Z])\d', code)
        if match:
            return match.group(1)

        # Fallback: any uppercase ICD-like letter
...
# ─── Dual-encoding colour utilities ──────────────────────────────────────────
# Colour = chapter HUE (categorical family) × admission INTENSITY (lightness)
# Diagnosis cells: high admissions → deep saturated hue; low → pale tint of same hue
# Chapter header cells: always use the full saturated base colour
...
def chapter_tinted(hex_color: str, t: float) -> str:
    """Return the chapter colour blended with white.
    t = 0.0 → very pale tint (low admissions)
    t = 1.0 → full saturated hue (highest admissions in chapter)
    A minimum mix of 20 % preserves the hue even for the smallest cells.
    """
    MIN_MIX = 0.20   # palest: 20 % colour + 80 % white
    MAX_MIX = 1.00   # richest: 100 % colour
...
def readable_text_color(hex_color: str) -> str:
    """Return '#0f172a' (dark) or 'white' whichever is more legible on hex_color."""
    r, g, b = _hex_to_rgb(hex_color)
    # Perceived luminance (sRGB)
    lum = 0.2126 * r + 0.7152 * g + 0.0722 * b
    return '#0f172a' if lum > 0.45 else 'white'

# Pre-compute per-chapter admission ranges for normalisation
# (used later when building treemap nodes)
...
# ─── ADDED: helper for chapter colour legend ──────────────────────────────────
def chapter_legend_html(color_map: dict) -> str:
    items = []
    for chapter in sorted(color_map.keys()):
    ...
    Each colour represents one ICD chapter in Figure 1.
    </span>
...
def get_dual_color(chapter: str, total: float) -> str:
    ...
    # Use a gamma curve so mid-range cells look clearly differentiated
    t = t ** 0.65
    return chapter_tinted(base, t)


# ── Label visibility: show label if cell ≥ 0.4% of total ─────────────────────
# (lower than before → more labels visible)
grand_total = tree_df['total'].sum()
chapter_totals = tree_df.groupby('chapter')['total'].sum()
MIN_DIAG_FRACTION = 0.004   # 0.4 % threshold (was 0.8 %)

tree_df['show_label'] = tree_df['total'] / grand_total >= MIN_DIAG_FRACTION
tree_df['is_top10'] = tree_df['label_full'].isin(top10_burden_set)

# Top-10 always show label (regardless of size)
...
for chapter, ch_total in chapter_totals.sort_values(ascending=False).items():
    ...
    # Chapter header = full saturated base colour
...
for _, row in tree_df.iterrows():
    ...
    # ── Dual colour: chapter hue + admission intensity ──
    cell_color = get_dual_color(chapter, total)
    txt_col = readable_text_color(cell_color)
    # Top-10: yellow-tinted label to stand out regardless of background
    if is_top10:
        txt_col = '#fef08a'

    rank_label = f"★ Top-10 Burden Rank #{top10_rank[full_name]}" if is_top10 else ""
    # Admission share info for hover
    share_pct = total / grand_total * 100
    hover_extra = f"{rank_label}<br>Share of all admissions: {share_pct:.2f}%" if rank_label else f"Share of all admissions: {share_pct:.2f}%"
    ...
    # Top-10: thicker dark border to make them pop
...
# ─── ADDED: colour legend for treemap ─────────────────────────────────────────
st.markdown(chapter_legend_html(CHAPTER_COLORS), unsafe_allow_html=True)
...
# Optional debug panel for checking the fix
with st.expander("Debug — chapter parsing check", expanded=False):
    debug_df = df[
        df["description"].astype(str).str.contains(
            "labour|delivery|pregnan|childbirth", case=False, na=False)
    ][["code", "code_clean_letter", "description", "chapter", "Admissions"]].copy()
